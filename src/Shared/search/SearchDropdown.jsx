...
export function SearchDropdown({
  ...
  const fetchGovernoratesData = async () => {
    try {
      ...
      setFilteredOptions(fetchedGovernorates.result);
    } catch (error) {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = governorates.filter(
      (governorate) =>
        governorate.name_en.toLowerCase().includes(term.toLowerCase()) ||
        governorate.name_ar.toLowerCase().includes(term.toLowerCase())
    );
    setFilteredOptions(
      govNum > 0
