...
export default function UserMenu() {
  ...
  const userMenus = [
    {
      languages: { english: "Profile", arabic: "الصفحةالشخصية" },
      href: "/profile",
      id: 1,
    },
    { languages: { english: "Favorite", arabic: "المفضلة" }, href: "/", id: 2 },
    {
      languages: { english: "Setting", arabic: "الإعدادات" },
    ...
    <Fragment>
      {userMenus.map((userMenu) => (
        <li key={userMenu.id} className={` w-48`}>
          ...
          >
            <ul className="">
              {userMenu.languages.english == "Profile" ? (
                <MdAccountCircle />
              ) : (
                ""
              )}
              {userMenu.languages.english == "Favorite" ? (
                <MdOutlineFavorite />
              ) : (
                ""
              )}
              {userMenu.languages.english == "Setting" ? (
                <MdOutlineSettings />
              ) : (
                ""
              )}
            ...
            <ul className="mx-3">
              {languageIs
                ? userMenu.languages.english
                : userMenu.languages.arabic}
            </ul>
          </Link>
        </li>
      ))}
      ...
      <li className={`w-48`}>
        ...
        >
          <Link className="flex  items-center" href="/signin">
            ...
            <ul className="mx-3">{languageIs ? "Log Out" : "خروج"}</ul>
          </Link>
        </button>
      </li>
    </Fragment>
