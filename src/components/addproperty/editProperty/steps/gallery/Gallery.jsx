...
const Gallery = ({ propertyDetils, setData }) => {
  ...
  const handleDeleteMultiImage = (index) => {
    const updatedMultiImage = Array.from(propertyDetils.album);
  ...
  const handleAddMultiImage = (e) => {
    ...
    if (
      newFiles.length +
        (propertyDetils.album ? propertyDetils.album.length : 0) >
      14
    ) {
      setImgMaxError(true);
    } else {
      ...
      if (propertyDetils.album) {
        const updatedMultiImage = [
          ...Array.from(propertyDetils.album),
          ...newFiles,
        ];
        setData({
          ...propertyDetils,
        ...
        <div className="w-full border-[3px]  rounded-lg flex flex-col justify-center gap-4 items-center min-h-[200px] p-5 bg">
          ...
          {propertyDetils?.album && (
            <div className="flex justify-center flex-wrap gap-4 ">
              {Array.from(propertyDetils.album).map((image, index) => (
                <div
                  key={index}
